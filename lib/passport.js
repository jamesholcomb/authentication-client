...
module.exports = class Passport {
  ...
  authenticate (credentials = {}) {
    ...
    return getCredentials.then(credentials => {
      return this.connected(app).then(socket => {
        ...
        const emit = app.io ? 'emit' : 'send';
        return this.authenticateSocket(credentials, socket, emit);
      });
    }).then(payload => {
  ...
  authenticateSocket (credentials, socket, emit) {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket[emit]('authenticate', credentials, (error, data) => {
        ...
        this.setJWT(data).then(() => {
          debug('Socket authenticated!');
          app.emit('socket-authenticated');
          resolve(data);
        });
      });
    });
  ...
  setJWT (data) {
    ...
    if (accessToken) {
      ...
      return Promise.resolve(this.app.get('storage').setItem(this.options.storageKey, accessToken))
        .then(() => data);
    }
